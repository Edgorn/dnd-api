...
import DenyJoinCampaign from "./denyJoinCampaign.use-case";
...
describe("DenyJoinCampaign UseCase", () => {
  it("should deny a pending join request when the caller is the master", async () => {
    const mockCampaignService = {
      findActiveById: vi.fn().mockResolvedValue(baseCampaign),
      denyJoinRequest: vi.fn().mockResolvedValue(joinResult),
    } as unknown as CampaignService;

    const useCase = new DenyJoinCampaign(mockCampaignService);
    const result = await useCase.execute(joinInput);

    expect(mockCampaignService.findActiveById).toHaveBeenCalledWith(campaignId);
    expect(mockCampaignService.denyJoinRequest).toHaveBeenCalledWith(joinInput);
  ...
  it("should throw NotFoundError when the campaign does not exist", async () => {
    ...
    expect(mockCampaignService.denyJoinRequest).not.toHaveBeenCalled();
  });
  ...
  it("should throw NotFoundError when persistence returns null", async () => {
    const mockCampaignService = {
      findActiveById: vi.fn().mockResolvedValue(baseCampaign),
      denyJoinRequest: vi.fn().mockResolvedValue(null),
    } as unknown as CampaignService;
