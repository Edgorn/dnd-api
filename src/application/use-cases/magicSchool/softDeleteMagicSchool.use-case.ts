...
export default class SoftDeleteMagicSchool {
  constructor(
  ...
  async execute(id: string, userId: string): Promise<void> {
    ...
    if (system.publisher !== userId) {
      throw new AppError("No tienes permisos para eliminar esta escuela de magia", 403);
    }

    await this.magicSchoolService.softDelete(id);
  }
}
