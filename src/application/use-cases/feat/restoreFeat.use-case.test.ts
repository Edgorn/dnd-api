...
import RestoreFeat from "./restoreFeat.use-case";
...
describe("RestoreFeat UseCase", () => {
  it("should restore a feat when the user is the system publisher", async () => {
    ...
    const useCase = new RestoreFeat(mockFeatService, mockSystemService);
    await useCase.execute("feat-1", "user-1");

    expect(mockFeatService.getById).toHaveBeenCalledWith("feat-1");
    expect(mockFeatService.restore).toHaveBeenCalledWith("feat-1");
  });
  ...
  it("should throw 404 when the feat does not exist", async () => {
    ...
    expect(mockFeatService.restore).not.toHaveBeenCalled();
  });
