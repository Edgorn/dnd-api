...
export default class SoftDeleteArmorType {
  constructor(
  ...
  async execute(id: string, userId: string): Promise<void> {
    ...
    if (system.publisher !== userId) {
      throw new AppError("No tienes permisos para borrar este tipo de armadura", 403);
    }

    await this.armorTypeService.softDelete(id);
  }
}
