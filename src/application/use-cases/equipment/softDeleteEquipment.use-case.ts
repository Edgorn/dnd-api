...
export default class SoftDeleteEquipment {
  constructor(
  ...
  async execute(id: string, userId: string): Promise<void> {
    ...
    if (system.publisher !== userId) {
      throw new AppError("No tienes permisos para borrar este equipamiento", 403);
    }

    await this.equipmentService.softDelete(id);
  }
}
