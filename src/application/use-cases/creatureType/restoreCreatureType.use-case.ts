...
export default class RestoreCreatureType {
  constructor(
  ...
  async execute(id: string, userId: string): Promise<void> {
    ...
    if (system.publisher !== userId) {
      throw new AppError("No tienes permisos para restaurar este tipo de criatura", 403);
    }

    await this.creatureTypeService.restore(id);
  }
}
